...
class ComputationalGraph():
	def __init__(self, n_heads, n_layers, empty=False):
	...
	def build_ioi_computational_graph(self):
		# TODO: add resid post and resid pre, maybe MLPs?
		g = {} # receiver -> sender
		# create heads subgraph
		heads = [(i, j) for i in range(self.n_layers) for j in range(self.n_heads)]
		for send in heads:
			for rec in heads:
				if send[0] < rec[0]:
					...
					rec_hook_name = utils.get_act_name("result", rec_layer)
					rec_hook_index = (slice(None), slice(None), rec_head, slice(None))
					sender = HookNode(send_hook_name, send_hook_index)
					receiver = HookNode(rec_hook_name, rec_hook_index)
	...
	def visualize(self, filename='out'):
		...
		for node, edges in self.graph.items():
			dot.node(str(node), str(node))  # Add each node to the graph
			for edge in edges:
				dot.edge(str(node), str(edge))  # Add directed edge from node to each edge

		dot.render(filename, view=True)  # Set view=True to open the graph automatically after creation
...
# 	print(corrupted_src_activation.shape)
# 	print(clean_src_activation.shape)
# 	if hook.layer() == receiver[0]:
# 		activation[:, :, receiver[1], :] += (corrupted_src_activation - clean_src_activation)
# 	return activation
		
class ACDC():
	def __init__(self, model, n_heads, n_layers, clean_cache, corrupted_cache, clean_dataset, corrupted_dataset, clean_logits):
		self.model = model
		self.G = ComputationalGraph(n_heads, n_layers)
		self.H = ComputationalGraph(n_heads, n_layers)
		self.clean_cache = clean_cache
		self.corrupted_cache = corrupted_cache
		self.n_heads = n_heads
		self.n_layers = n_layers
		self.clean_dataset: IOIDataset = clean_dataset,
	...
	def ablate_paths(
		...
		# if clean_cache is None or clean_logits is None: # we need to keep the logits to compare to the patched logits
		# 	clean_logits, clean_cache = model.run_with_cache(
		# 		clean_dataset.toks,
		# 		names_filter = sender_hook_names_filter,
		# 		return_type='logits'
		# 	)
...
if MAIN:
	...
	# result_name_filter = lambda name: name.endswith('z')
	...
	for i, src in enumerate(srcs):
		for j, rec in enumerate(receivers):
			...
			print()
			print(src)
			results[i_results][j_results] += remove_path_head_to_head(src, rec, "v", model, orig_cache=orig_cache, orig_logits=orig_logits)
